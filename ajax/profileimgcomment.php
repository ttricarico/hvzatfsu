	...
	***********************/
	...
	function newpost()
	{
		global $cxn;
			...
			$query = "SELECT hvzid FROM uploadedimages WHERE imagename='".$imgname."'";
			$result = mysqli_query($cxn, $query);
			$info = mysqli_fetch_assoc($result);
			mysqli_free_result($result);
			
			$query = "INSERT INTO notifications(hvzid, notiftime, bywhohvzid, bywhoname, notifwhere, notiftext, notiflink)
									VALUES('".$info['hvzid']."', '".time()."', '".$posterhvzid."', '".$postername."', 'proimg', '".$postername." has commented a photo of yours.', 'profile.php?view=pic&p=".$imgname."&hvzid=".$info['hvzid']."')";
			$result = mysqli_query($cxn, $query);	
