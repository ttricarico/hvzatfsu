...
function delimgcomment(postid)
{
	$.ajax({
		type: "POST",
		url: "apps/ajax_profileimgcomment.php?action=deletepost",
		data: "post=" + postid,
		success:function(){
...
function submitcaptiontxt()
{
	...
	function aboutmechg()
	{
		...
		$("#aboutme").html('<textarea id="aboutmetext" onblur="sendaboutme();">' + aboutme + '</textarea>');
	
	}
	function sendaboutme()
	{
		var aboutmetxt = $("#aboutmetext").val();
		$("#editaboutme").html('<img src="images/icons/redloader.gif" title="Loading..." />');
		...
		$.ajax({
				...
				success: function(){
					$("#aboutme").text(aboutmetxt);
					$("#aboutmetext").removeAttr("disabled");
					$("#editaboutme").html('Edit About Me');
				}
			});
	}
	$(document).ready(function(){
