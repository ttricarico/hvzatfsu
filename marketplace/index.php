...
<div id="frame">
    ...
    <div id="left">
        <div id="recentitems">
           <h3>Recently Posted Items:</h3>
            	<?php
			$query = "SELECT * FROM marketplace WHERE 1 ORDER BY timepost DESC LIMIT 0,12";
			$result = mysqli_query($cxn, $query);
			if(mysqli_num_rows($result) == 0)
			{echo "There are no items for sale at the moment. You can <a href=\"javascript://\" class=\"newitemlink\">post a new item</a>";}
			else {
				...
				while($item = mysqli_fetch_array($result))
				{
					if($i%3 == 0)
					{ echo "<tr>".PHP_EOL; }
					echo "<td align=\"center\"><table border=\"0\" id=\"items\" width=\"100%\">
						<tr><td>Price: $".$item['price']."</td></tr>
						<tr><td><a href=\"viewitem.php?itemid=".$item['id']."\"><img src=\"apps/uploads/thumbnails/".$item['image']."\" border=\"0\" /></a></td></tr>
						<tr><td><a href=\"viewitem.php?itemid=".$item['id']."\">".$item['title']."</a></td></tr>
						</table></td>";
					if($i%3 == 2)
					{ echo "</tr>".PHP_EOL; }
        ...
        <div id="randomitems">
            <h3>Random items:</h3>
		<?php
			$query = "SELECT * FROM marketplace WHERE 1 ORDER BY RAND() LIMIT 12";
			$result = mysqli_query($cxn, $query);
			if(mysqli_num_rows($result) == 0)
			{	echo "There are no items for sale. <a href=\"javascript://\" class=\"newitemlink\">Post a new item</a>"; }
			else {
				echo "<table width=\"100%\" border=\"0\">";
				$i = 0;
				while($item = mysqli_fetch_array($result))
				{
					...
					echo "<td><table border=\"0\">
						<tr><td>Price: $".$item['price']."</td></tr>
						<tr><td><img src=\"apps/uploads/thumbnails/".$item['image']."\" border=\"0\" /></td></tr>
						<tr><td><a href=\"viewitem.php?itemid=".$item['id']."\">".$item['title']."</a></td></tr>
						</table></td>";
					if($i%3 == 2)
					{ echo "</tr>".PHP_EOL; }
